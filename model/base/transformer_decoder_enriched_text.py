...
class transformer_decoder(nn.Module):
  ...
  def __init__(self, args, num_queries, hidden_dim, dim_feedforward, nheads=4, num_layers=1, pre_norm=False):
    ...
    # === FIXED === removed unused `transformer_ffn_layers` (was never appended/called)
    ...
    # === FIXED === moved out of the for loop. Previously these were inside
    # the loop, which (a) silently re-created supp_q_feat with new random
    # weights every iteration when num_layers > 1, and (b) was misleading.
    # Behaviorally identical when num_layers == 1.
  ...
  def forward(self, x, x_s, support_mask, text_tokens=None, text_padding_mask=None):
    ...
    for i in range(self.num_layers):
        ...
        # === FIXED === added `self.use_text_cross_attention` guard so that
        # accidental text_tokens injection without enabling fusion does not
        # cause an IndexError (the layer list is empty in that case).
        if text_tokens is not None and self.use_text_cross_attention:
