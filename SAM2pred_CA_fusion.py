...
class  SAM_pred(nn.Module):
    def __init__(self, checkpoint=None, feature_cache=None, benchmark='coco'):
        super().__init__()
        if checkpoint is None:
            checkpoint = os.environ.get(
                'VRPSAM_SAM_CHECKPOINT',
                os.path.join(os.path.dirname(os.path.abspath(__file__)), 'checkpoints', 'sam_vit_h_4b8939.pth'),
            )
        if feature_cache is None:
            feature_cache = os.environ.get(
                'VRPSAM_SAM_FEATURE_CACHE',
                os.path.join(os.path.dirname(os.path.abspath(__file__)), 'feats_np'),
            )
        self.feature_cache = os.path.join(feature_cache, benchmark)
        self.sam_model = sam_model_registry['vit_h'](checkpoint)
        self.sam_model.eval()
    ...
    def get_feat_from_np(self, query_img, query_name, protos):
        np_feat_path = self.feature_cache
        ...
        for idx, name in enumerate(query_name):
            ...
            if name + '.npy' not in files_name:
                ...
                np.save(os.path.join(np_feat_path, name + '.npy'), query_feats_np)
            else:
                sub_query_feat = torch.from_numpy(np.load(os.path.join(np_feat_path, name + '.npy'))).to(protos.device)
