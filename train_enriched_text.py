...
from model.VRP_encoder_enriched_text import VRP_encoder
from common.logger import Logger, AverageMeter
from common.evaluation import Evaluator
from common import utils
from data.dataset_enriched_text import FSSDataset
...
def train(args, epoch, model, sam_model, dataloader, optimizer, scheduler, training):
    ...
    for idx, batch in enumerate(dataloader):
        ...
        # === MODIFIED === pass already-formatted text prompts from dataloader.
        # `batch.get('text_prompt')` returns:
        #   - None when --text_prompt_json is not set (model ignores text branch)
        #   - List[str] of len=B when training
        #   - List[List[str]] of shape [B, K] when validating (K-prompt ensemble)
        protos, _ = model(
            args.condition,
            batch['query_img'],
...
if __name__ == '__main__':
    ...
    # === MODIFIED === text_prompt_template removed (dataloader now supplies
    # formatted prompts via pascal_text_prompts.json). Four new args added:
    parser.add_argument('--text_prompt_json', type=str, default=None,
                        help='Path to pascal_text_prompts.json. '
                             'Required when --fusion_type text_cross_attention.')
    parser.add_argument('--text_p_template', type=float, default=0.5,
                        help='Prob of sampling from templates vs descriptions (training).')
    parser.add_argument('--text_use_synonyms', action='store_true',
                        help='Enable synonym replacement for text augmentation.')
    parser.add_argument('--text_syn_prob', type=float, default=0.3,
                        help='Prob of synonym replacement when --text_use_synonyms is set.')
    # =================
    args = parser.parse_args()

    # === ADDED === fail fast on incompatible text-config combinations
    if args.fusion_type == 'text_cross_attention' and not args.text_prompt_json:
        raise ValueError(
            "--text_prompt_json is required when --fusion_type='text_cross_attention'. "
            "Example: --text_prompt_json data/pascal_text_prompts.json"
        )
    # =================
    ...
    # === MODIFIED === pass text-prompt config into FSSDataset
    FSSDataset.initialize(
        img_size=512,
        datapath=args.datapath,
        use_original_imgsize=False,
        text_prompt_json=args.text_prompt_json,
        text_p_template=args.text_p_template,
        text_use_synonyms=args.text_use_synonyms,
        text_syn_prob=args.text_syn_prob,
    )
    # =================
