...
# === ADDED === optional text prompt database
...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize,
        ...
        # === ADDED === text database (optional; None = original behavior)
    ...
    def __getitem__(self, idx):
        ...
        # === ADDED === inject text prompt field if text_db is provided
        if self.text_db is not None:
            if self.split == 'trn':
                # training: a single randomly-sampled prompt (str)
                batch['text_prompt'] = self.text_db.sample_train(class_name)
            else:
                # eval: list of all templates + descriptions for ensemble
                # NOTE: with batch_size > 1, default_collate will "transpose"
                # this into List[List[str]]. The model side must handle
                # whatever collate produces.
                batch['text_prompt'] = self.text_db.get_eval_ensemble(class_name)
        # ==============
